...
*   1D plots include histograms and density estimates
*   2D plots are most often scatterplots
*   for higher dimensions, biplots are often the most useful
...
In the `ggplot2` framework, biplots can be produced by the `ggbiplot` package, but this package is not on
CRAN, so cannot be directly used here. Instead, the code below was run locally and the result
included.
```{r biplot1}
#| eval=FALSE
...
### Predicting crime: Multiple regression

### Predicting crime: MANOVA
